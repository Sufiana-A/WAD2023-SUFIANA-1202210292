...
$tinggiBadan = isset($_POST['tinggi_badan']) ? $_POST['tinggi_badan'] :null;
...
if ($tinggiBadan == null || $beratBadan == null) {
    $message = "Tinggi Badan atau Berat Badan Ga Boleh Kosong Nih Nilainya";
} 
...
if ($message == "") {
    ...
    $rest_bmi = ($beratBadan / (($tinggiBadan)*($tinggiBadan)));
    if ($rest_bmi <= 18.4) {
        $status = "Underweight";
    } else if ($rest_bmi > 18.4 and $rest_bmi <= 24.9) {
        $status = "Normal";
    } else if ($rest_bmi >= 25 and $rest_bmi <= 39.9) {
        $status = "Overweight";
    } else if ($rest_bmi >= 40) {
        $status = "Obese";
    }
